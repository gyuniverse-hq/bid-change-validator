...
structured output violates the observed product context.
"""
...
7. analysis_status가 PARTIAL이면 판정의 직접 원인이 없다는 식으로 말하지 않는다. 대신 자동 검토 범위가 완전하지 않다고만 설명한다.
8. company_profile은 판정 당시 snapshot이며, 제공된 필드만 사용한다. 식별자를 추정하거나 복원하지 않는다.
9. key_points는 입력에 있는 requirement_key만 사용한다. 판정 밖 수동 확인사항을 요약할 때만 requirement_key를 null로 둔다.
10. 저장·반영·재검증을 실행했다고 말하지 않는다. next_action은 조회·확인 안내만 작성한다.
11. 짧고 대화체로 답한다. 시스템 내부 용어, JSON, reason_code를 그대로 노출하지 않는다.
"""
...
def _manual_review(summary: QualificationSummary) -> list[str]:
    scope = summary.analysis_scope
    if scope is None:
        return []
    items = [item.message for item in scope.notice_facts]
    items.extend(item.raw for item in scope.dropped_requirements if item.raw.strip())
    return items[:8]
...
def _recompose(result: CopilotChatResponse, narration: ProductNarration, allowed_keys: set[str]) -> CopilotChatResponse:
...
def apply_product_narration(
    ...
    *,
    ...
    """Narrate read-only Product Truth and fail closed to the deterministic answer."""
    ...
    try:
        ...
        if narration.status != summary.overall_status:
            return result
        return _recompose(result, narration, allowed_keys)
    except Exception:
        # Narration is an optional presentation layer. Provider failures,
        # validation failures or unsafe target selection never replace product truth.
        return result
