...
try {
  ...
  await page.locator('#copilot-question').fill('현장 방문은 완료했지만 확인서는 아직 제출하지 않았어');
  const nextResponse = page.waitForResponse(r=>r.url().endsWith('/copilot/chat'),{timeout:160000});
  await page.getByRole('button',{name:'질문 보내기',exact:true}).click();
  const next = await nextResponse;
  const nextBody = await next.json();
  responses.push({status:next.status(),body:nextBody});
  result.followupRequest = JSON.parse(next.request().postData());
  result.proposalCount = nextBody.envelope.actions.length;
  assert.equal(next.status(),200);
  assert(nextBody.envelope.actions.length > 0, 'NO_CONFIRMATION_PROPOSAL_FOR_VISIT_ANSWER');
  const action=nextBody.envelope.actions[0];
  assert.equal(action.action_type,'ANSWER_REQUIREMENT');
  assert.equal(action.user_input.satisfies_requirement,false);
  assert.equal(action.user_input.evidence_held,false);
  assert.deepEqual(JSON.parse(action.user_input.normalized_value).answers,{site_visited:true,visit_certificate:false});
  assert.equal(nextBody.envelope.processing.task_status,'PASS');
  await page.locator('[data-copilot-version="3.1"]').last().waitFor();
  await page.getByRole('button',{name:'서버 제안 검토 · 아직 실행 안 함',exact:true}).last().click();
  await page.waitForURL('**/ask-back?**');
  await page.getByRole('button',{name:'내용 확인 후 실행',exact:true}).waitFor();
  result.reviewScreen='visible; execution button not clicked';
