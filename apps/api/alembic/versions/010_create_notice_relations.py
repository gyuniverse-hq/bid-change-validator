...
def upgrade() -> None:
    ...
    # Existing snapshots already retain the upstream field in raw_json. Backfill
    # the latest known direct relation per notice, even when the previous notice
    ...
    op.execute(
        ...
        SELECT
            latest.notice_id,
            CASE
                WHEN previous.id = latest.notice_id THEN NULL
                ELSE previous.id
            END,
