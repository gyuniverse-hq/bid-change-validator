...
import { ANALYSIS_STATUS_COPY } from '@/lib/status-copy';
    ...
    <section className="app-shell-container pb-14">
      <div className="border-t border-[var(--product-line)] pt-10">
        <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
          <div><p className="text-[12px] font-bold text-[var(--product-accent-deep)]">프로필 기준 매칭 · 분석 완료 공고만</p><h2 className="mt-1 text-[27px] font-extrabold tracking-[-0.035em]">회사 기준으로 판정 가능한 공고</h2><p className="mt-2 text-[13px] text-[var(--product-muted)]">분석이 끝난 공고의 자격요건을 회사 프로필과 대조했습니다. 아직 분석하지 않은 공고는 여기 없습니다.</p></div><span className="text-[12px] text-[var(--product-muted)]">분석 완료 {matches.length}건</span>
        </div>
        {error && <p className="mt-4 rounded-xl bg-rose-50 px-4 py-3 text-[13px] text-rose-700">{error}</p>}
        {matches.length ? <div className="mt-5 grid gap-4 lg:grid-cols-3">{matches.slice(0, 6).map((item) => { const meta = STATUS[item.overall_status]; return <article key={item.notice_id} className="rounded-[20px] border border-[var(--product-line)] bg-white p-5"><div className="flex items-center justify-between"><span className={`rounded-full px-3 py-1 text-[12px] font-bold ${meta.className}`}>{meta.label}</span><span className="text-[11px] text-[var(--product-muted)]">{ANALYSIS_STATUS_COPY[item.analysis_status as keyof typeof ANALYSIS_STATUS_COPY]?.label ?? item.analysis_status}</span></div><h3 className="mt-4 line-clamp-2 text-[16px] font-bold leading-6">{item.title}</h3><p className="mt-2 text-[12px] text-[var(--product-muted)]">{item.institution_name ?? '기관 미상'} · {item.bid_notice_no}</p>{item.satisfied_count + item.unknown_count + item.unsatisfied_count > 0 ? <div className="mt-4 flex gap-3 text-[12px]"><span className="text-emerald-700">충족 {item.satisfied_count}</span><span className="text-amber-700">확인 {item.unknown_count}</span><span className="text-rose-700">미달 {item.unsatisfied_count}</span></div> : <p className="mt-4 text-[12px] leading-5 text-[var(--product-muted)]">{item.requirement_count === 0 ? '공고에서 자격요건을 찾지 못해 비교하지 못했습니다.' : '자격요건을 찾았지만 판정된 항목이 없습니다.'}</p>}<Button size="sm" className="mt-5 rounded-full" onClick={() => void openReview(item)} disabled={busy !== null}>{busy === item.notice_id ? <LoaderCircle className="animate-spin" /> : null} 검토 열기 <ArrowRight /></Button></article>; })}</div> : <div className="mt-5 rounded-[20px] border border-dashed border-[var(--product-line)] px-6 py-10 text-center text-[13px] text-[var(--product-muted)]">아직 현재 공고 중 자격요건 분석이 완료된 후보가 없습니다. 공고 검토를 실행하면 캐시된 분석 결과가 이 영역에 누적됩니다.</div>}
      </div>
    </section>
  );
}
