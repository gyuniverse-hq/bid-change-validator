...
let result = {status:'FAIL',scope:'Real Chrome, persistent local accounts/API/DB; one live profile-summary question; not human evaluation'};
try {
  await page.goto('http://127.0.0.1:5181/login');
  await page.waitForFunction(() => !document.querySelector('button[type="submit"]')?.disabled);
  await page.locator('#username').fill(accounts[0].username);
  await page.locator('#password').fill(accounts[0].password);
  await page.locator('button[type="submit"]').click();
  await page.waitForURL('**/company');
  await page.goto(accounts[0].url);
...
console.log('PASS: local login -> company -> qualification -> opt-in -> live profile summary');
