...
def compose_document_ledger(bundle, plan, gateway):
    ...
    scope_policy += ('부분 원문에 질문의 정보가 없다는 이유로 공고 전체 또는 다른 부분에도 없다고 말하지 않는다. '
                     '관련 정보가 없는 부분은 relevant=false로 분류한다. 관련 정보가 있으면 그 내용만 설명하며 '
                     '"이 부분에는 마감일이 없다" 같은 조각별 부재 안내를 덧붙이지 않는다. '
                     '특정 제출서류의 별도 제출기한이 명시되지 않은 한계는 그 서류를 명확히 지칭한다. ')
