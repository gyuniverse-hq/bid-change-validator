"""판정 결과 JSON -> 초안 생성기 프롬프트 문자열. 모델은 쓰지 않는다.

여기는 조판만 한다. 판정은 이미 `rules.judge_requirements` 가 끝냈고, 이 모듈은
그 결과를 사람이 읽는 줄로 엮을 뿐이며 해석하거나 요약하지 않는다.
...
이 파일의 자리
--------------
평가 하네스(`quality_eval/business_plan/briefing_format.py`, PR #122)에 같은
내용이 있다. 제품이 평가 코드를 import 하는 것은 방향이 거꾸로라 제품 쪽에
원본을 두고, #122 가 머지되면 그쪽이 이 파일을 import 하도록 바꾼다.
"""
...
# 판정 레코드에는 산문 reason 이 없고 reason_code 만 있다. status-copy.ts 와 같은 문구.
REASON_CODE_LABEL = {
...
def render_briefing(
    ...
    *,
    ...
    for index, judgment in enumerate(judgments, start=1):
        ...
        reason = (judgment.get("reason") or "").strip() or REASON_CODE_LABEL.get(
            judgment.get("reason_code") or "", ""
        )
        if reason:
