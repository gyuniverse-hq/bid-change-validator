...
(async () => {
  ...
  try {
    ...
    await panel.getByRole('button', { name: '내용 확인 후 실행' }).waitFor();
    ...
    await panel.getByRole('button', { name: '내용 확인 후 실행' }).click();
    await panel.getByText('반영은 완료됐지만 새 판정을 불러오지 못했습니다. 결과 조회만 다시 시도해 주세요.', { exact: true }).waitFor();
    assert.equal(count, 1);
    failRefresh = false;
    await panel.getByRole('button', { name: '결과 조회만 다시 시도' }).click();
    await panel.getByText('반영 후 새 판정 결과를 확인했습니다.', { exact: true }).waitFor();
    assert.equal(count, 1);
