...
export type CopilotEnvelope = {
  ...
  sources: { source_id: string; kind: 'DOCUMENT' | 'PRODUCT' | 'TURN' | 'PROCEDURE'; quote: string;
  ...
  follow_up_targets: { target_id: string; kind: string; label: string; message_id: string;
    ...
    origin_tool?: string | null; entity_ref?: string | null }[];
  capabilities: Record<string, number>; actions: ActionProposal[];
  limitations: string[]; clarification: string | null;
  job?: { goal: string; status: 'OPEN' | 'COMPLETE'; requirements: {
    requirement_id: string; request: string; reason: string;
...
export function validateEnvelope(envelope: CopilotEnvelope, caseId?: string) {
  ...
  if (new Set(envelope.claims.map(claim => claim.claim_id)).size !== envelope.claims.length ||
    sources.size !== envelope.sources.length || envelope.claims.some(claim =>
