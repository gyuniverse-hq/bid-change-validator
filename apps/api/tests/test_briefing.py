...
from apps.api.app.ai.qualification.extraction.analysis_result import AnalysisDiagnostic
...
def test_evidence_can_include_the_full_source_chunk_for_frontend_expansion() -> None:
    source_text = "2. 참가자격\n최근 3년 이내 실적 5억원 이상을 보유한 업체"
    briefing = _briefing(
        source_text_by_chunk_id={"CHUNK-0002": source_text},
    )

    assert briefing.judgments[0].evidence[0].source_text == source_text
