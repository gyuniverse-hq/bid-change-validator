...
let finishFollowup;
const overlapping = new ConversationStore(() => new Promise(resolve => { finishFollowup = resolve; }));
const followup = overlapping.ask('case-overlap', '방금 반영한 답변을 설명해줘');
const pendingRevision = overlapping.get('case-overlap').revision;
overlapping.publish('case-overlap', copilotMocks.eligible);
assert.equal(overlapping.get('case-overlap').revision, pendingRevision);
assert.equal(overlapping.get('case-overlap').busy, true);
overlapping.publish('case-overlap', copilotMocks.eligible);
assert.equal(new Set(overlapping.get('case-overlap').turns.map(t => t.id)).size, 3);
finishFollowup(copilotMocks.productEvidence);
await followup;
assert.equal(overlapping.get('case-overlap').busy, false);
assert.equal(overlapping.get('case-overlap').turns.at(-1).response, copilotMocks.productEvidence);
assert.equal(overlapping.get('case-overlap').turns.at(-1).question, '방금 반영한 답변을 설명해줘');
assert.equal(overlapping.get('case-other').turns.length, 0);
