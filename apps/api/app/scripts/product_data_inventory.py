...
from ..judgment_models import QualificationJudgment, QualificationJudgmentRun
from ..models import (
    BidNotice,
    BidNoticeVersion,
    Company,
...
def collect_product_data_inventory(db: Session) -> dict[str, object]:
    version_rows = db.execute(
        select(BidNoticeVersion.notice_id, func.count(BidNoticeVersion.id))
        .group_by(BidNoticeVersion.notice_id)
    ).all()
    version_distribution = Counter(int(count) for _, count in version_rows)

    document_download_status = dict(
        db.execute(
            select(NoticeDocument.download_status, func.count(NoticeDocument.id))
            .group_by(NoticeDocument.download_status)
        ).all()
    )
    document_extraction_status = dict(
        db.execute(
            select(NoticeDocument.extraction_status, func.count(NoticeDocument.id))
            .group_by(NoticeDocument.extraction_status)
        ).all()
    )
    collection_status = dict(
        db.execute(
            select(NoticeCollectionRun.status, func.count(NoticeCollectionRun.id))
            .group_by(NoticeCollectionRun.status)
        ).all()
    )
    ...
    return {
        "notices": {
            ...
            "with_multiple_versions": sum(count for versions, count in version_distribution.items() if versions >= 2),
            "version_distribution": {str(versions): count for versions, count in sorted(version_distribution.items())},
        },
        "documents": {
            "count": _count(db, NoticeDocument),
            "download_status": {str(key): int(value) for key, value in document_download_status.items()},
            "extraction_status": {str(key): int(value) for key, value in document_extraction_status.items()},
        },
        "collection_runs": {
        ...
        "qualification": {
            ...
            "judgments": _count(db, QualificationJudgment),
