        ...
        <div className="mt-4 space-y-4">
          ...
          {unaskable.map((question) => {
            ...
            return (
              <section key={question.requirement_key} className="rounded-[20px] border border-[#eef0f4] bg-white px-[26px] py-6">
                ...
                {evidence && <div className="mt-4"><EvidenceQuote quote={evidence.quote} location={evidence.location} /></div>}
                {evidence && (
                  <NavigationLink href={`${workspaceHref('/evidence', workspace.caseItem.id)}&evidence=${encodeURIComponent(evidence.key)}`} className={buttonVariants({ variant: 'outline', className: 'mt-4 rounded-full' })}>근거 원문에서 확인</NavigationLink>
                )}
              </section>
            );
          })}
          ...
          {totalUnknown === 0 && (
            <section className="rounded-[20px] border border-[#eef0f4] bg-white px-[26px] py-12 text-center">
              ...
              <NavigationLink href={workspaceHref('/qualification', workspace.caseItem.id)} className={buttonVariants({ variant: 'outline', className: 'mt-5 rounded-full' })}>참가자격 검토로 돌아가기</NavigationLink>
            </section>
          )}
        </div>
      </div>
