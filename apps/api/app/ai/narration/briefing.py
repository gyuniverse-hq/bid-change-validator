...
풀어쓰는 것뿐이다. 충족/미충족은 `app.qualification.rules.judgment`가 코드로 정했고, 조항 위험은
...
from ..qualification.extraction.analysis_result import (
    AnalysisDiagnostic,
...
def _quote(
    evidence: Evidence,
    source_text_by_chunk_id: dict[str, str] | None = None,
) -> EvidenceQuote:
    return EvidenceQuote(
        ...
        source_text=(source_text_by_chunk_id or {}).get(evidence.chunk_id or ""),
    )
...
def build_briefing(
    *,
    ...
    for judgment in judgments:
        ...
        quotes = [
            _quote(evidence_by_key[key], source_text_by_chunk_id)
