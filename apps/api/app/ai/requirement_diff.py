...
Requirement keys are extraction-order based, so matching uses exact keys first and
then a conservative semantic identity fallback. Only decision-relevant fields are
compared; evidence locator changes alone do not force re-judgment.
"""
...
def decision_payload(req: QualificationRequirement)->dict:
    return {"type":req.type,"operator":req.operator,"value":req.value,"unit":req.unit,"period_months":req.period_months,"scope":req.scope,"required":req.required,"group_operator":req.group_operator}
...
def diff_requirements(baseline:list[QualificationRequirement], current:list[QualificationRequirement])->list[RequirementChange]:
    ...
    for key in sorted(set(baseline_by_key)&set(current_by_key)):
        ...
        if b.type==c.type:
            pairs.append((b,c,f"key:{key}")); matched_base.add(b.requirement_key); matched_current.add(c.requirement_key)
    base_fallback={}
    for b in baseline:
