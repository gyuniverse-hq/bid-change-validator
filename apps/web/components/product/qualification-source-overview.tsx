      ...
      <section className="mt-7">
        <div className="rounded-[20px] border border-[var(--product-line-2)] bg-white p-6">
          ...
          <div className="mt-4 divide-y divide-[var(--product-line-2)]">
            {version?.documents.length ? version.documents.map((document) => <div key={document.id} className="flex items-center gap-3 py-3"><div className="min-w-0 flex-1"><strong className="block truncate text-[15px]">{document.name}</strong><span className="mt-1 block text-[12px] text-[var(--product-muted)]">{labelOf(VIEWER_TYPE_LABEL, document.viewer_type)} · {labelOf(EXTRACTION_STATUS_LABEL, document.extraction_status)}{document.extracted_char_count != null ? ` · ${document.extracted_char_count.toLocaleString()}자` : ''}</span></div><Button variant="outline" size="sm" className="rounded-full" render={<Link href={`/evidence?caseId=${caseItem.id}`} />}>원문 대조</Button></div>) : <p className="py-6 text-center text-[15px] text-[var(--product-muted)]">현재 차수에 수집된 문서가 없습니다.</p>}
          </div>
        </div>
      </section>
    </>
