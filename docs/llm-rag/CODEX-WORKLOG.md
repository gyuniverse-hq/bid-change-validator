...
## 2026-09-13 — 추출 재현율 개선 원인 검수

- `feat/extraction-recall-harness`의 청크 경계, clause safety 장식 제거, 생산 근거
  정규화 문제를 코드와 테스트로 검수했다.
- 세 원인 자체는 확인했지만 청크 rank 반례와 괄호 내 실제 조건 삭제 위험을 발견해
  현 구현을 그대로 승격하기에는 보완이 필요하다고 판단했다.
- 특수점(`·`/`․`) 차이로 생산 검증이 탈락하는 현상은 직접 재현했다.
- 20공고 정량 결과는 fixture bundle과 결과 JSON이 저장소에 없어 재현하지 못했으며,
  보고 내 `39→54`와 `39→56` 수치도 서로 불일치한다.
- 상세: `docs/llm-rag/2026-09-13-extraction-recall-audit.md`

### 검수 반례 후속 수정

- 청크 제목 위계를 세분화해 `3.1 → 가.`와 괄호형 숫자 자격 제목을 올바르게
  선별하도록 수정했다.
- clause safety는 괄호 전체 삭제를 중단하고, 주소 증빙 설명만 제한적으로 제거하며
  괄호 안 실제 공동수급·부정·대안 조건은 계속 차단하도록 보강했다.
- 생산 근거 비교에서 `·`/`․` 등 호환 문장부호를 정규화하되 응답·DB의 `raw` 원문은
  그대로 보존하도록 수정했다.
- 추출·safety·판정·canonical·통합·quality-eval 관련 테스트 `85 passed`를 확인했다.
- 최초 검수 시 저장소 내부에는 fixture bundle과 결과 artifact가 없어 20공고 성능
  수치를 재검증할 수 없었다. 이후 제공된 외부 경로로 아래 재실행을 완료했다.

### 외부 골든셋 실제 재실행

- 외부 경로 `C:\Users\playdata2\협업자료-3차프로젝트\golden_runner_v02`에서
  `fixture_bundle.json`과 기존 실행 결과를 확인했다.
- 코드 전용 판정 40 cases는 기준 commit의 `104 일치 / 34 안전 보류`에서 현재
  `110 일치 / 28 안전 보류`로 바뀌었고, 양쪽 모두 잘못된 확정 0이다.
- 사용자의 외부 전송 승인 후 공용 Supabase 원문을 읽어 OpenAI 구조화 추출을 20공고
  각 1회 실행했다. DB 쓰기는 하지 않았다.
- 현재 결과는 `18/59 도달(30.5%)`, `20 버림`, `10 미분류`, `11 놓침`이다.
  직전 결과도 `18/59`였으므로 전체 도달률 개선은 확인되지 않았다.
- `RAW_NOT_FOUND_IN_SOURCE`는 6→5로 줄었지만 `DETAIL_NOT_FOUND_IN_SOURCE` 15건은
  그대로여서, 특수점 정규화가 21건 탈락 전체의 원인이라는 설명은 틀렸다.
- 결과는 `docs/llm-rag/results/product_engine_actual_current.json`과
  `docs/llm-rag/results/extraction_recall_live_20260913T094132Z.json`에 저장했다.
